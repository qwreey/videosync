...
const code = await run.main(async () => {
  ...
  // members is a count; a regex over the whole status found a 2 in "127.0.0.1" and could not fail.
  check('A sees two members', memberCount(members) === 2, `members ${memberCount(members)}: ${JSON.stringify(members).slice(0, 200)}`);
