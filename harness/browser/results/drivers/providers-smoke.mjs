...
const out = { when: new Date().toISOString(), server: SERVER, flags: '-providers <dir: localmedia.json>', checks: [], steps: [] };
const check = (name, ok, detail) => { out.checks.push({ name, ok: !!ok, detail }); console.log(`${ok ? 'PASS' : 'FAIL'}  ${name}  ${detail ?? ''}`); };
const step = (s) => { out.steps.push({ t: new Date().toISOString(), ...s }); console.log('  ', JSON.stringify(s).slice(0, 400)); };
...
console.log(`\n${out.checks.filter((c) => c.ok).length}/${out.checks.length} passed`);
writeFileSync(new URL('../providers-smoke.json', import.meta.url), JSON.stringify(out, null, 1));
process.exit(0);
