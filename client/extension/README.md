...
The worker holds no session state: it is a frame relay, an HTTP relay (see
Permissions), plus the provider bookkeeping that needs its privileges (reading
granted hosts, registering the content script for added sites). The engine, the
detector, the clock and the adapter all live in the content script beside the
`<video>`, sharing `client/core/src/app/bootstrap.ts` verbatim with the
userscript.
