...
2. **클라이언트 진단**:
   - 문제가 난 탭에서 개발자 도구 → 콘솔 위쪽의 실행 컨텍스트를 `top`에서 **VideoSync**
     (확장 이름)로 바꾼다.
   - `copy(VideoSync.dump())`를 실행하고 붙여 넣는다.
   - 들어 있는 것: 엔진 상태, 최근 프레임 기록, 플레이어 상태, 프로바이더 정보.
   - **토큰과 방 비밀키는 들어 있지 않다.** 프레임 기록은 정해진 필드(seq, 위치, 종류, 미디어 키·URL 등)만 남기고,
     주소창의 초대 비밀키는 가려진다. 문제가 난 **직후에** 뽑아도 된다. 기록은 항상 남고 있다.
...
userscript는 개발자 도구에서 컨텍스트를 고르기 어렵다. 재현이 필요한 문제는 확장으로 다시 해 본다.
