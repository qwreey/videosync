...
|---|---|---|
| YouTube works via the generic adapter | **SETTLED — yes** (BROWSER-FINDINGS §8): `play()` accepted, `currentTime` sticks, `playbackRate` 1.1 held for 10 s. No `movie_player` adapter needed | — |
| `playbackRate` nudging is safe on an MSE player | **SETTLED — yes** (BROWSER-FINDINGS §7, §8) on hls.js and on YouTube | Laftel still open |
| Chrome MV3 service worker can hold a WebSocket | **SETTLED — yes** (BROWSER-FINDINGS §10): 10 minutes, one instance, zero closes, with traffic *and* idle | — |
...
- **A page on a public origin cannot reach a loopback or private address at all** — any scheme, and
  the request never leaves the browser (BROWSER-FINDINGS §8). This is why the userscript needs a public name with a
  certificate and the extension does not.
- **An extension's service worker is exempt from that** (BROWSER-FINDINGS §9), which is the strongest argument for
